            
            <section id="grids">

                <h1>Grid examples</h1>

                <hr class="hr__big">

                <!-- Single 'column' layout ie. no grid class used -->
                <h2>H2 title</h2>
                <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

                <hr>

                <!-- 2 column grid layout -->
                <h2>2 Column responsive grid</h2>
	            <div class="grid__col--2">
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	            </div>

	            <hr>

	            <!-- 3 column grid layout -->
                <h2>3 Column responsive grid</h2>
	            <div class="grid__col--3">
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	            </div>

	            <hr>

	            <!-- 4 column grid layout -->
                <h2>4 Column responsive grid</h2>
	            <div class="grid__col--4">
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	            </div>

	            <hr>

	            <!-- 5 column grid layout -->
                <h2>5 Column responsive grid</h2>
	            <div class="grid__col--5">
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	            </div>

	            <hr>

	            <!-- 6 column grid layout -->
	            <h2>6 Column responsive grid</h2>
	            <div class="grid__col--6">
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	                <div class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </div>
	            </div>

	            <hr>

	            <!-- 6 column grid layout (using a list) -->
	            <h2>6 Column responsive grid</h2>
	            <p>(Using an <code>&lt;ol&gt;</code> or <code>&lt;ul&gt;</code> list)</p>
	            <ul class="grid__col--6">
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	                <li class="col">
	                    <h3>H3 title</h3>
	                    <p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	                </li>
	            </ul>

	            <hr>

	            <!-- 2 Column responsive grid - (with nested 2 column grid) -->
	            <h2>2 Column responsive grid <small>- (with nested 2 column grid)</small></h2>
	            <ul class="grid__col--2">
	            	<li class="col">
	            		<h3>H3 title</h3>
	            		<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

	            		<!-- Nested grid -->
	            		<div class="grid__col--2">
	            			<div class="col">
	            				<h3>H3 title</h3>
	            				<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	            			</div>
	            			<div class="col">
	            				<h3>H3 title</h3>
	            				<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	            			</div>
	            		</div>
	            		<!-- / Nested grid -->
	            	</li>
	            	<li class="col">

	            		<!-- Nested grid -->
	            		<div class="grid__col--2">
	            			<div class="col">
	            				<h3>H3 title</h3>
	            				<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	            			</div>
	            			<div class="col">
	            				<h3>H3 title</h3>
	            				<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>
	            			</div>
	            		</div>
	            		<!-- / Nested grid -->

	            		<h3>H3 title</h3>
	            		<p>In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat. In platea urna, turpis in. Porttitor dictumst. Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

	            	</li>
	            </ul>

	            <hr>

	            <h2>Equal height columns / boxes solution.</h2>

                <ul class="grid__col--5 col__equal-wrap">
				    <li class="col col__equal-item">
				    	<article class="media__stacked post-sum">
			            	<a href="" class="media__image">
			            		<img src="static/img/posts/drums-650x365.jpg" alt="">
			            	</a>
			            	<div class="media__body">
			                    <header class="post-sum__header">
			                        <!-- Article title as a link. Typically link through to full article / news post -->
			                        <h1 class="post-sum__title">
			                            <a href="">An article header title link</a>
			                        </h1>
			                        <h2 class="post-sum__title-sub">Sub heading</h2>
			                        <!-- Post meta data -->
			                        <small class="post-sum__meta">
			                            <time datetime="2016-05-12T10:00">
			                                <!-- Date -->
			                                <span class="post-sum__date">21<sup>st</sup> May 2016</span>
			                                <!-- Time -->
			                                <span class="post-sum__time">10am - 4pm</span>
			                            </time>
			                            <!-- Extra info -->
			                            <span class="post-sum__extra">Free</span>
			                        </small>
			                    </header><!-- /header -->

			                    <!-- Article summary copy -->
			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <!-- Primary link to this article / post -->
		                    	<a href="" class="link__standalone">Read this article</a>

		                    </div>

		                </article>

				    </li>
				    <li class="col col__equal-item">
				    	<article class="media__stacked post-sum">
			            	<a href="" class="media__image">
			            		<img src="static/img/posts/drums-650x365.jpg" alt="">
			            	</a>
			            	<div class="media__body">
			                    <header class="post-sum__header">
			                        <!-- Article title as a link. Typically link through to full article / news post -->
			                        <h1 class="post-sum__title">
			                            <a href="">An article header title link</a>
			                        </h1>
			                        <h2 class="post-sum__title-sub">Sub heading</h2>
			                        <!-- Post meta data -->
			                        <small class="post-sum__meta">
			                            <time datetime="2016-05-12T10:00">
			                                <!-- Date -->
			                                <span class="post-sum__date">21<sup>st</sup> May 2016</span>
			                                <!-- Time -->
			                                <span class="post-sum__time">10am - 4pm</span>
			                            </time>
			                            <!-- Extra info -->
			                            <span class="post-sum__extra">Free</span>
			                        </small>
			                    </header><!-- /header -->

			                    <!-- Article summary copy -->
			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <!-- Primary link to this article / post -->
		                    	<a href="" class="link__standalone">Read this article</a>

		                    </div>

		                </article>

				    </li>
				    <li class="col col__equal-item">
				    	<article class="media__stacked post-sum">
			            	<a href="" class="media__image">
			            		<img src="static/img/posts/drums-650x365.jpg" alt="">
			            	</a>
			            	<div class="media__body">
			                    <header class="post-sum__header">
			                        <!-- Article title as a link. Typically link through to full article / news post -->
			                        <h1 class="post-sum__title">
			                            <a href="">An article header title link</a>
			                        </h1>
			                        <h2 class="post-sum__title-sub">Sub heading</h2>
			                        <!-- Post meta data -->
			                        <small class="post-sum__meta">
			                            <time datetime="2016-05-12T10:00">
			                                <!-- Date -->
			                                <span class="post-sum__date">21<sup>st</sup> May 2016</span>
			                                <!-- Time -->
			                                <span class="post-sum__time">10am - 4pm</span>
			                            </time>
			                            <!-- Extra info -->
			                            <span class="post-sum__extra">Free</span>
			                        </small>
			                    </header><!-- /header -->

			                    <!-- Article summary copy -->
			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <!-- Primary link to this article / post -->
		                    	<a href="" class="link__standalone">Read this article</a>

		                    </div>

		                </article>

				    </li>
				    <li class="col col__equal-item">
				    	<article class="media__stacked post-sum">
			            	<a href="" class="media__image">
			            		<img src="static/img/posts/drums-650x365.jpg" alt="">
			            	</a>
			            	<div class="media__body">
			                    <header class="post-sum__header">
			                        <!-- Article title as a link. Typically link through to full article / news post -->
			                        <h1 class="post-sum__title">
			                            <a href="">An article header title link</a>
			                        </h1>
			                        <h2 class="post-sum__title-sub">Sub heading</h2>
			                        <!-- Post meta data -->
			                        <small class="post-sum__meta">
			                            <time datetime="2016-05-12T10:00">
			                                <!-- Date -->
			                                <span class="post-sum__date">21<sup>st</sup> May 2016</span>
			                                <!-- Time -->
			                                <span class="post-sum__time">10am - 4pm</span>
			                            </time>
			                            <!-- Extra info -->
			                            <span class="post-sum__extra">Free</span>
			                        </small>
			                    </header><!-- /header -->

			                    <!-- Article summary copy -->
			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <!-- Primary link to this article / post -->
		                    	<a href="" class="link__standalone">Read this article</a>

		                    </div>

		                </article>

				    </li>
				    <li class="col col__equal-item">
				    	<article class="media__stacked post-sum">
			            	<a href="" class="media__image">
			            		<img src="static/img/posts/drums-650x365.jpg" alt="">
			            	</a>
			            	<div class="media__body">
			                    <header class="post-sum__header">
			                        <!-- Article title as a link. Typically link through to full article / news post -->
			                        <h1 class="post-sum__title">
			                            <a href="">An article header title link</a>
			                        </h1>
			                        <h2 class="post-sum__title-sub">Sub heading</h2>
			                        <!-- Post meta data -->
			                        <small class="post-sum__meta">
			                            <time datetime="2016-05-12T10:00">
			                                <!-- Date -->
			                                <span class="post-sum__date">21<sup>st</sup> May 2016</span>
			                                <!-- Time -->
			                                <span class="post-sum__time">10am - 4pm</span>
			                            </time>
			                            <!-- Extra info -->
			                            <span class="post-sum__extra">Free</span>
			                        </small>
			                    </header><!-- /header -->

			                    <!-- Article summary copy -->
			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <p>Massa elementum habitasse eros cursus, nunc, natoque porta mus pellentesque, proin tortor, integer lundium ac mauris integer risus, a non elit nunc lundium, quis dapibus risus et! Porta, mus etiam, lorem nascetur nec urna penatibus placerat.</p>

			                    <!-- Primary link to this article / post -->
		                    	<a href="" class="link__standalone">Read this article</a>

		                    </div>

		                </article>

				    </li>
				</ul>

	        </section>
